...
const InviteFriendScreen = ({navigation, route}) => {
    ...
    const IndividualFriend = ({userID, id, name})=> {
        return(
          <TouchableOpacity style={{alignSelf: 'center', width: '100%', height: 75, paddingVertical: 12, paddingHorizontal: 24, flexDirection: 'row'}}>
    ...
    return(
        <SafeAreaView style={styles.container}> 
            <View style={{position: 'absolute', width: '100%', height: 60, top: 45, flexDirection:'row', paddingTop: 20}}>
                <TouchableOpacity onPress={()=>gotoMakeFolderBottomSheetScreen({navigation, folderUserIDs})} 
                                style={{left: 21, width: 20, alignItems:'center', height: 20, justifyContent: 'center'}}>
