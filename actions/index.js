...
export const fetchAllFolder2 = async (UID) => {
  fetchAllFolderObject(UID).then((allFolderObject) => {
    Object.entries(allFolderObject).forEach(([folderID,]) => {
      const folderObject = fetchFolder(folderID)
      console.log(folderObject)
      //console.log(folderObject)
      allFolderObject[folderID] = folderObject
