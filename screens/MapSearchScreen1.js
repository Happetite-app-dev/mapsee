...
const MapSearchScreen1 =({ navigation })=>{
  ...
  const onClick = (dataSource) => {
    console.log(3)
    gotoSearch3Screen({navigation: navigation, data: [name, dataSource]});
  }
  ...
  return (
      <SafeAreaView style={styles.container}>
          ...
          <GooglePlacesAutocomplete
              getSearchWord={(text) => {setName(text)}}
              getResultArray={(dataSource) => {onClick(dataSource), console.log(2)}}
              textInputProps={{
              ...
              onPress={(data, details) => {
                  ...
                  const newPlace = {
                    lctn:{latitude: details.geometry.location.lat, longitude: details.geometry.location.lng}, 
                    name: details.name, address: details.formatted_address, id: details.place_id, 
                  ...
                  gotoSearch2Screen({navigation: navigation, data: details})
              }}

              onFail={error => console.log(error)}
              onNotFound={() => console.log('no results')}

              styles={{textInputContainer: {backgroundColor: 'grey',},textInput: {height: 38,color: '#5d5d5d',fontSize: 16,},
