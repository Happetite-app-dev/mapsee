...
import { getDatabase, ref, onValue, set, push, DataSnapshot } from 'firebase/database';
...
const callFriendRequest = (friendUID, myUID, myID, myFirstName, myLastName) => {
    if(friendUID!=undefined){
      ...
      push(reference, {                                   //db에 myUID, 요청 시각, type: "newfriend"가 적힌 obj가 friendUID/request에 새롭게 적힘}
      ...
      SendPushNotification({receiverUID: friendUID, title_: 'tit', body_: 'bd'})
    }
    else{
...
const AddFriendModal = ({modalVisible: modalVisible, modalHandler: modalHandler}) => {
  ...
  return (
    <View style={styles.centeredView}>
      <Modal
        ...
        onRequestClose={() => {        //추후에 없앨 필요 있을 수도(ios)
      ...
      >
        <View style={styles.centeredView}>
          <View style={styles.modalView}>
            <View style={{marginTop: 16, width: 312, height: 48, alignItems: "center", borderBottomWidth: 1, borderBottomColor: 'gray'}}>
              <TextInput 
            ...
            <View style={{flexDirection: 'row', justifyContent: 'space-between', width: 300, height: 48}}>
              <Pressable
                style={{marginTop: 8,...styles.button, ...styles.buttonClose}}
              ...
              <Pressable
                style={{marginTop: 8,...styles.button, ...styles.buttonClose}}
                onPress={() => {getFriendUID(newFriend, handleFriendUID); modalHandler(!modalVisible);}}
              >
